...
> **nvblox 실행 절차 본체(§6 이후)는 이 문서가 아니라 cuMotion bringup 절차이 단일 출처다** — 가장 최신이고 실패 이력(`std::lerp`, `warp.torch`)까지 있다.
> 이 문서는 §1~§5(워크스페이스·udev·Docker 컨테이너 셋업)만 유효하다. 문서 지도: [문서 지도](README.md)
